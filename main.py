...
import pytz  # Thêm cho thời gian thực
from dateutil.relativedelta import relativedelta  # Thêm cho tính toán ngày
...
server_members = {
    "demacianking1": {
        ...
        "last_countdown_wish": None,  # Lưu ngày đếm ngược gần nhất
        "countdown_sent": []  # Lưu các ngày đếm ngược đã gửi
    },
    "thanh0374": {
        "name": "Thành",
        "full_name": "Thành", 
...
def get_vietnam_time():
    ...
    try:
        ...
        # Fallback: dùng UTC+7 nếu không có pytz
...
# Hàm tóm tắt drama từ chat history (TÓM TẮT TOÀN BỘ 500 TIN NHẮN)
...
# Hàm gửi thông báo đếm ngược sinh nhật
...
async def check_birthdays_and_countdown(client):
    ...
    try:
        ...
        current_minute = vn_time.minute
        
        print(f"🕒 [Sinh nhật] Kiểm tra lúc {vn_time.strftime('%d/%m/%Y %H:%M:%S')} (VN Time)")
        ...
        for username, info in server_members.items():
            ...
            # Nếu sinh nhật đã qua trong năm nay, tính sang năm sau
...
async def on_message(message):
    ...
    # Lệnh Tarot (CẬP NHẬT: gửi ảnh + giải thích)
    ...
    if client.user.mentioned_in(message) or isinstance(message.channel, discord.DMChannel):
        try:
            async with message.channel.typing():
                ...
                # Lấy thông tin thời gian hiện tại (VN Time)
