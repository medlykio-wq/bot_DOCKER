...
# Thông tin thành viên server với tính năng đếm ngược sinh nhật
server_members = {
    "demacianking1": {
        "name": "Cường",
        "full_name": "Cường",
...
# Hàm lấy thời gian Việt Nam thực tế
...
# ==============================================
# HÀM SINH NHẬT (ĐÃ SỬA QUAN TRỌNG)
# ==============================================

async def send_countdown_message(client, username, info, days_left, birthday_date):
    """Gửi thông báo đếm ngược sinh nhật - ĐÃ SỬA"""
    try:
        user = None
        ...
        if user:
            age_at_birthday = birthday_date.year - info['year']
            
            if days_left == 1:
                message = f"🎉 **CHỈ CÒN 1 NGÀY NỮA** là đến sinh nhật của {info['name']}! 🎂\n"
                message += f"Ngày mai ({birthday_date.strftime('%d/%m')}) {user.mention} sẽ tròn **{age_at_birthday} tuổi**! 🥳\n"
                message += f"🎁 Mọi người chuẩn bị quà đi nào! ✨"
            elif days_left == 0:
                message = f"🎉 **HÔM NAY LÀ SINH NHẬT** của {info['name']}! 🎂\n"
                message += f"Hôm nay ({birthday_date.strftime('%d/%m')}) {user.mention} tròn **{age_at_birthday} tuổi**! 🥳🎊"
            else:
                message = f"🎉 **CÒN {days_left} NGÀY** nữa là đến sinh nhật của {info['name']}! 🎂\n"
                message += f"Ngày {birthday_date.strftime('%d/%m')} {user.mention} sẽ tròn **{age_at_birthday} tuổi**! ⏰\n"
                message += f"🎈 Đang đếm ngược... 🎈"
                ...
                for channel in guild.text_channels:
                    if channel.permissions_for(guild.me).send_messages:
                        try:
                            ...
                            print(f"❌ Lỗi khi gửi đến channel {channel.name}: {e}")
                            continue
            
            print(f"⚠️ Không tìm thấy channel phù hợp để gửi cho {info['name']}")
            return False
            
    except Exception as e:
        print(f"❌ Lỗi khi gửi đếm ngược: {e}")
        return False

async def send_birthday_wish(client, username, info, age, today):
    """Gửi lời chúc mừng sinh nhật - ĐÃ SỬA"""
    try:
        user = None
        for guild in client.guilds:
            user = guild.get_member_named(username)
                ...
                for channel in guild.text_channels:
                    if channel.permissions_for(guild.me).send_messages:
                        try:
                            ...
                            print(f"🎂 Đã gửi chúc mừng sinh nhật cho {info['name']} ({age} tuổi)")
                            return True
                        except Exception as e:
                            print(f"❌ Lỗi khi gửi đến channel {channel.name}: {e}")
                            continue
            
            print(f"⚠️ Không tìm thấy channel phù hợp để gửi chúc mừng cho {info['name']}")
            return False
...
async def check_birthdays_and_countdown(client, force_check=False):
    """Kiểm tra sinh nhật và gửi thông báo đếm ngược - ĐÃ SỬA QUAN TRỌNG"""
    try:
        vn_time = get_vietnam_time()
        today = vn_time.date()
        current_hour = vn_time.hour
        
        print(f"\n🕒 [Sinh nhật] Kiểm tra lúc {vn_time.strftime('%d/%m/%Y %H:%M:%S')} (VN Time)")
        
        for username, info in server_members.items():
            birth_day = info["birthday"]["day"]
            birth_month = info["birthday"]["month"]
            birth_year = info["year"]
            ...
            # KIỂM TRA LUÔN, KHÔNG CHỈ 9:00 SÁNG
            should_check = True
            
            if should_check:
                # 1. KIỂM TRA SINH NHẬT HÔM NAY (ngày 0) - ƯU TIÊN CAO NHẤT
                if days_until_birthday == 0:
                    last_wish = info.get("last_birthday_wish")
                    if last_wish != today.strftime("%Y-%m-%d") or force_check:
                        print(f"     🎉 PHÁT HIỆN SINH NHẬT HÔM NAY! Đang gửi chúc mừng...")
                        success = await send_birthday_wish(client, username, info, age_at_birthday, today)
                        if success:
                            info["last_birthday_wish"] = today.strftime("%Y-%m-%d")
                            info["countdown_sent"] = []
                        else:
                            print(f"     ❌ Không thể gửi chúc mừng sinh nhật cho {info['name']}")
                
                # 2. KIỂM TRA ĐẾM NGƯỢC (1, 2, 3, 4, 5 ngày trước) - ĐÃ SỬA
                elif 1 <= days_until_birthday <= 5:
                    countdown_key = f"{today.strftime('%Y-%m-%d')}_{days_until_birthday}"
                    
                    if countdown_key not in info.get("countdown_sent", []) or force_check:
                        print(f"     ⏰ PHÁT HIỆN ĐẾM NGƯỢC {days_until_birthday} NGÀY! Đang gửi thông báo...")
                        success = await send_countdown_message(client, username, info, days_until_birthday, next_birthday)
                        if success:
                            info["countdown_sent"] = info.get("countdown_sent", []) + [countdown_key]
                        else:
                            print(f"     ❌ Không thể gửi đếm ngược cho {info['name']}")
                
                # 3. KIỂM TRA NẾU QUÁ 5 NGÀY, RESET DANH SÁCH
                elif days_until_birthday > 5:
                    if info.get("countdown_sent"):
                        info["countdown_sent"] = []
                        print(f"     🔄 Reset danh sách đếm ngược cho {info['name']}")
            
            if days_until_birthday <= 10:
                print(f"     📅 Ngày sinh nhật: {next_birthday.strftime('%d/%m/%Y')}")
                print(f"     🎂 Tuổi: {age_at_birthday}")
                if days_until_birthday == 0:
                    print(f"     🎉 HÔM NAY LÀ SINH NHẬT!")
                elif days_until_birthday <= 5:
                    print(f"     ⏰ Đếm ngược: {days_until_birthday} ngày")
...
async def show_member_info(username, channel):
    """Hiển thị thông tin thành viên - ĐÃ SỬA"""
    if username in server_members:
        info = server_members[username]
        vn_time = get_vietnam_time()
        today = vn_time.date()
        ...
        this_year_birthday = datetime.date(today.year, info["birthday"]["month"], info["birthday"]["day"])
        if today > this_year_birthday:
            next_birthday = datetime.date(today.year + 1, info["birthday"]["month"], info["birthday"]["day"])
        else:
            next_birthday = this_year_birthday
        
        days_until_birthday = (next_birthday - today).days
        age_at_next_birthday = next_birthday.year - info["year"]
        
        countdown_status = "❌ Chưa gửi"
        if days_until_birthday <= 5 and days_until_birthday >= 0:
            countdown_key = f"{today.strftime('%Y-%m-%d')}_{days_until_birthday}"
            if countdown_key in info.get("countdown_sent", []):
                countdown_status = "✅ Đã gửi"
            else:
                countdown_status = "⏳ Chờ gửi"
...
🕒 **Sinh nhật tiếp theo:** {next_birthday.strftime('%d/%m/%Y')}
⏰ **Còn lại:** {days_until_birthday} ngày
🎉 **Sẽ tròn:** {age_at_next_birthday} tuổi
📊 **Trạng thái đếm ngược:** {countdown_status}
...
"""
        ...
        if days_until_birthday == 0:
            ...
            if last_wish == today.strftime("%Y-%m-%d"):
                response += f"\n✅ **Bot đã chúc mừng vào:** {last_wish}"
            else:
                response += f"\n❌ **Chưa chúc mừng** (Bot sẽ tự động gửi)"
        elif days_until_birthday <= 5:
            response += f"\n🎁 **Đang đếm ngược:** Bot sẽ thông báo ngay! ⏰"
...
# ==============================================
# DISCORD CLIENT - ĐÃ SỬA PHẦN VÒNG LẶP
# ==============================================
...
async def on_ready():
    ...
    print("🔍 Đang kiểm tra sinh nhật ngay khi khởi động...")
    await check_birthdays_and_countdown(client, force_check=True)
    
    client.loop.create_task(birthday_check_loop())

async def birthday_check_loop():
    """Vòng lặp kiểm tra sinh nhật - CHẠY THƯỜNG XUYÊN HƠN"""
    ...
    while not client.is_closed():
        try:
            ...
            should_check = (
                current_hour in [9, 12, 18, 0] or
                check_count % 4 == 0
            )
            
            if should_check:
                print(f"\n🔄 [Vòng lặp {check_count}] Kiểm tra sinh nhật...")
...
async def on_message(message):
    ...
    if message.content and not message.author.bot:
        timestamp = get_vietnam_time().strftime("%H:%M")
    ...
    # Lệnh kiểm tra sinh nhật ngay lập tức (MỚI)
    if message.content.startswith('!check_birthdays'):
        await message.channel.send("🔍 Đang kiểm tra sinh nhật ngay lập tức...")
        await check_birthdays_and_countdown(client, force_check=True)
        await message.channel.send("✅ Đã kiểm tra xong! Kiểm tra console/log để xem kết quả.")
        return
    
    # Lệnh gửi chúc mừng sinh nhật thủ công (MỚI)
    if message.content.startswith('!force_birthday'):
        ...
        if len(parts) == 2:
            ...
            if username in server_members:
                ...
                await message.channel.send(f"🎯 Đang gửi chúc mừng sinh nhật cho {info['name']}...")
                success = await send_birthday_wish(client, username, info, age, today)
                if success:
                    server_members[username]["last_birthday_wish"] = today.strftime("%Y-%m-%d")
                    await message.channel.send(f"✅ Đã gửi chúc mừng sinh nhật cho {info['name']}!")
                else:
                    await message.channel.send(f"❌ Không thể gửi chúc mừng sinh nhật cho {info['name']}")
            else:
                await message.channel.send(f"❌ Không tìm thấy username: {username}")
        else:
            await message.channel.send("❌ Cú pháp: `!force_birthday username`")
        return
    
    # Lệnh test countdown sinh nhật
    if message.content.startswith('!test_countdown'):
        parts = message.content.split()
        if len(parts) == 2:
            username = parts[1]
            if username in server_members:
                ...
                # Simulate 5 days countdown
                for days in [5, 4, 3, 2, 1, 0]:
                    await message.channel.send(f"🔧 **TEST**: Còn {days} ngày đến sinh nhật {info['name']}")
                    await asyncio.sleep(1)
                
                # Test actual birthday
    ...
    if client.user.mentioned_in(message) or isinstance(message.channel, discord.DMChannel):
        try:
            async with message.channel.typing():
                ...
                time_context = f"Thời gian hiện tại: {current_time.strftime('%H:%M %d/%m/%Y')} (VN Time)\n"
